	...
	const onAddToCart = async obj => {
		...
		try {
			...
			if (findItem) {
				...
				/**Cначала показываю пользователю результат, а потом отправляю запрос 
				...
				/**Тут у нас проблема возникла. В стэйт не передается id корзины, а передается id товара.
				 * Получается, необходимо дождаться пока запрос выполниться и передать данные с бекэнда в state
				 * Нужно сделать так, чтобы id из корзины брать который есть в корзине, а не тот, который передается на главной странице*/
			}
		} catch (error) {
		...
		*/}
			<div className='wrapp clear'>
				...
				<Routes>
					...
					/>
					<Route
						/**Здесь мы говорим, что данный Route  будет рендерится только в том случае если путь  '/favorites'
						 * И будет рендерится весь код из модуля favorites
						 * Мы можем как и где угодно рендерить Route.
						 * !!!ВАЖНО!!!
